var textEditor = {
    ...
    init: function (e) {
        ...
        $('#text-area').keyup(function (e) {
            ...
            if (e.originalEvent.ctrlKey == true && e.key == 'a') return;
            ...
            if (textEditor.canPushCurlyBracketClose == false && e.key == '}')
                ...
                e.preventDefault();
            else if (e.code == 'ControlLeft') return;
            else {
    ...
    organizeText: function (e, text) {
        if (text != "") {
            ...
            textEditor.arrangeHeight(e, markedHtmlString);
        }
        else {
    ...
    addSpacesToHtmlArray: function (e, htmlArr) {
        ...
        for (var i = 0; i < htmlArr.length; i++) {
            ...
            if (htmlArr[i] == '{' || htmlArr[i] == '[') {
                ...
                }//tab sayısını ayarlama

                if (i != 0)
                    resultArr.push(additionalString);//Başta ekleme yapmasın diye
                ...
                else if (htmlArr[i + 1] == '}' || htmlArr[i + 1] == ']') {
                    var additionalString2 = additionalString + "\t"; 

                    resultArr.push(additionalString2);
                }
                else {
    ...
    arrangeHeight: function (e, text) {
        ...
        if (count > 30) {
            $('#text-area').css('min-height', (800 + (count / 30) * 500).toString() + "px");
            $('#text').css('min-height', (800 + (count / 30) * 500).toString() + "px");
            $('#backdrop').css('min-height', (800 + (count / 30) * 500).toString() + "px");
        }
    },
    ...
    // #region arrangeCursor
    arrangeCursor: function (e, text) {
        $('#text-area').focus();

        if (textEditor.beforeText != null && textEditor.beforeText != '') {
            ...
            if (e.key == ',') {
                ...
                /*index = index + (Math.floor((text.length - textEditor.beforeText.length) / 2) );*/

                var a = textEditor.findDifferenceBetweenTwoText(textEditor.beforeText, text);

                var b = textEditor.findDifferenceOfCursorIndex(a);
            ...
            $('#text-area')[0].setSelectionRange(3, 3);
        }

        textEditor.beforeText = text;
    },

    findDifferenceBetweenTwoText: function(text1, text2) {
        var differentIndex = null;

        var bigText = "";
        var littleText = "";

        var differentCount = text1.length - text2.length;
        ...
        for (var j = 0; j < differentCount; j++) {
            result += bigText[differentIndex + j];
        }

        return result;
    },

    findDifferenceOfCursorIndex: function (text) {
        var isCursorOpen = false;
        var secondN = false;

        if (text.startsWith('\t{') || text.startsWith('\t[')) {
            for (var i = 0; i < text.length; i++) {
                if (text[i] == '{' || text[i] == '[') isCursorOpen = true;
